<?php
	...
	if(	!empty($_REQUEST['status']) ){
		...
		$cond .= ' AND a.status = "'.$value.'" ';
		$cond_html .= ' <b>Status:</b> '.$value;
	}
	$date_cond = '';
	...
	while($row = $db->fetchByAssoc($res)){
		...
		$tbl_rows .= '<tr>';
			...
			$tbl_rows .= '</td>';
				...
				$tbl_rows .= '</td>';			
			$tbl_rows .= '<td>';
			$tbl_rows .= '<a href="index.php?module=Leads&action=DetailView&record='.$row['id'].'">'.$row['first_name'].' '.$row['last_name'].'</a>';
			$tbl_rows .= '</td>';
				$tbl_rows .= '<td>';
				$tbl_rows .= $row['ufirst'].' '.$row['ulast'];
				$tbl_rows .= '</td>';				
			$tbl_rows .= '<td>';
