<?php
	...
	echo '<h3>Report: Leads by Source</h3><br/><br/>';
	
	
	//print_r($app_list_strings['lead_source_dom']);
...
<input type="hidden" name="action" id="action" value="lss">
...
if( isset($_REQUEST['category_id']) && !empty($_REQUEST['category_id'])  ){
	...
	//echo $cond_html .= ' <b>Category:</b> '.$categories[$category_id];
}
...
function getLeadSummaryData($users, $from, $till, $category_id){
	...
	$source_list = $app_list_strings['lead_source_dom'];
	...
	//init data schema
	$o_data = array();
	foreach($source_list as $_key => $_val){
	...
	if(isset($from)&&!empty($from)){
		...
		//$cond_html .= ' <b>From Date:</b> '.$from;
	}
	if(isset($till)&&!empty($till)){
		if( !empty($date_cond) ) $date_cond .= " AND ";
		$date_cond .= " $_datefield <= ".db_convert("'".$till." 23:59:59'", 'date');
		//$cond_html .= ' <b>Till Date:</b> '.$till;
	}
	
	//lead_source

	$sql = " SELECT a.lead_source, a.status, COUNT(a.id) as ecnt
	FROM leads as a
	...
	if(!empty($category_id)){
		...
		$productObj = BeanFactory::getBean('AOS_Product_Categories', $category_id);
		$cond_html .= ' <b>Category:</b> '.$productObj->name;
	}
	
	if( !empty($cond_html) ){
		echo '<br/><span style="color:red;">Filter:</span> '.$cond_html.'<br/>';
	}

	$sql .= " GROUP BY a.lead_source, a.status; ";//ORDER BY assigned_user_id desc
	$res = $db->query($sql);
	while($row = $db->fetchByAssoc($res)){
		$u_id = $row['lead_source'];
		//if( empty($u_id) ){
		//	continue;
		//}
	...
	foreach($o_data as $source_id => $stats){
		$open_cnt = 0;
		$cnt = 0;
		$cnt = sumUserTotal($stats);
		if($cnt > 0){
			$tbl_head .= '<tr>';
			$tbl_head .= '<td>'.$app_list_strings['lead_source_dom'][$source_id].'</td>';
				
			$tbl_head .= '<td><a target="_blank" href="index.php?module=AOR_Reports&action=leadSummaryDetail&source_id='.$source_id.'&category_id='.$category_id.'&from='.$from.'&till='.$till.'">'.$cnt.'</a></td>';
			foreach($stats as $status_key => $status_cnt){
				...
				if( !empty($status_key) ){
					...
					if($status_cnt > 0){
						...
						//}						
						$tbl_head .= '<td><a target="_blank" href="index.php?module=AOR_Reports&action=leadSummaryDetail&source_id='.$source_id.'&category_id='.$category_id.'&from='.$from.'&till='.$till.'&status='.$status_key.'">'.$status_cnt.$percent_str.'</a></td>';
					}else{
						$tbl_head .= '<td>-</td>';
					}
				}
			...
			//}
			$tbl_head .= '<td><a target="_blank" href="index.php?module=AOR_Reports&action=leadSummaryDetail&source_id='.$source_id.'&category_id='.$category_id.'&from='.$from.'&till='.$till.'&status=Dead">'.$stats['Dead'].$percent_str.'</a></td>';
			...
			//	$percent_str = '&nbsp;<span style="color:brown;">'.(number_format($tmp, 0, '', '')).'%</span>';
			//}
			$tbl_head .= '<td><a target="_blank" href="index.php?module=AOR_Reports&action=leadSummaryDetail&source_id='.$source_id.'&category_id='.$category_id.'&from='.$from.'&till='.$till.'&status=Converted">'.$stats['Converted'].$percent_str.'</a></td>';
