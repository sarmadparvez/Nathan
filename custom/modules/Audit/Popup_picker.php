...
class Popup_Picker
{
	...
	function process_page()
	{
		...
		$dataArr = [];
		$dataArr['name'] = $focus->name;
		$dataArr['date_entered'] = $focus->date_entered;
		$dataArr['created_by_name'] = $focus->created_by_name; 
        $xtpl=new XTemplate ('custom/modules/Audit/Popup_picker.html');
        $xtpl->assign('ADD', $dataArr);
		...
		if($field_count > 0)
		{
    		...
    		foreach($audited_fields as $key=>$value)
			{
				...
				if(isset($value['vname']))
					...
					$vname = $value['label'];
				$fields .= '<span class="floatleft">'.str_replace(':', '', translate($vname, $focus->module_dir)).'</span>';
    		...
    		echo '<h4>'.translate('LBL_AUDITED_FIELDS', 'Audit').'</h4><table cellpadding="0" cellspacing="0" width="100%" border="0" class="list view" style="table-layout:fixed"><tr><td></td></tr><tr><td>'.$fields.'</td></tr><tr><td></td></tr></table>';
    	}
    	else
    	{
    		echo '<h4>'.translate('LBL_AUDITED_FIELDS', 'Audit').'</h4>';
    	}
